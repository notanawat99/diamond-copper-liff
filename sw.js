...
body{font-family:'Sarabun',sans-serif;background:var(--bg);color:var(--text);min-height:100vh;padding-bottom:48px;overscroll-behavior-y:none;}
...
.photo-preview{width:100%;border-radius:8px;margin-top:8px;display:none;aspect-ratio:16/9;object-fit:cover;background:#1E293B;cursor:pointer;}
...
.cam-guide-frame{position:absolute;left:4%;right:4%;border:2px solid rgba(255,255,255,0.7);border-radius:12px;}
.cam-guide-label{position:absolute;left:0;right:0;text-align:center;color:rgba(255,255,255,0.85);font-size:13px;font-weight:700;font-family:'Sarabun',sans-serif;text-shadow:0 1px 3px rgba(0,0,0,0.9);}
.cam-bar{display:flex;align-items:center;justify-content:space-between;padding:16px 20px;background:#000;flex-shrink:0;}
...
<div class="cam-overlay" id="camOverlay">
  ...
  <div class="cam-guide" id="camGuide">
    <div class="cam-guide-frame" id="camGuideFrame"></div>
    <div class="cam-guide-label" id="camGuideLabel"></div>
  </div>
  <div class="cam-bar">
...
// ---- IMAGE COMPRESSION ----
// อ่าน EXIF orientation จาก binary ของ JPEG
...
function compressImage(dataUrl, maxSize=1200, quality=0.6){
  return new Promise(resolve=>{
    ...
    img.onload = ()=>{
      ...
      // swap dimensions สำหรับรูปที่หมุน 90/270
      const swapped = orientation>=5 && orientation<=8;
      let sw=swapped?h:w, sh=swapped?w:h;
      if(sw>maxSize||sh>maxSize){
        if(sw>sh){sh=Math.round(sh*maxSize/sw);sw=maxSize;}
        else{sw=Math.round(sw*maxSize/sh);sh=maxSize;}
      }
      const canvas=document.createElement('canvas');
      canvas.width=sw; canvas.height=sh;
      const ctx=canvas.getContext('2d');
      // หมุน canvas ตาม EXIF
      if(orientation===2){ctx.transform(-1,0,0,1,sw,0);}
      else if(orientation===3){ctx.transform(-1,0,0,-1,sw,sh);}
      else if(orientation===4){ctx.transform(1,0,0,-1,0,sh);}
      else if(orientation===5){ctx.transform(0,1,1,0,0,0);}
      else if(orientation===6){ctx.transform(0,1,-1,0,sh,0);}
      else if(orientation===7){ctx.transform(0,-1,-1,0,sh,sw);}
      else if(orientation===8){ctx.transform(0,-1,1,0,0,sw);}
...
function setupCamGuide(){
  const guide=document.getElementById('camGuide');
  const frame=document.getElementById('camGuideFrame');
  const label=document.getElementById('camGuideLabel');
  if(!camIsVehicle){guide.classList.remove('show');return;}
  const isPlate=camTargetId==='vP1';
  const vw=window.innerWidth;
  const vh=window.innerHeight;
  const barH=96;
  const availH=vh-barH;
  // frame กว้าง 92% ของหน้าจอ สูงตามสัดส่วน: ป้าย 35%, รถ 56%
  const frameW=vw*0.92;
  const frameH=isPlate?frameW*0.35:frameW*0.56;
  ...
  label.style.top=(frameTop+frameH+10)+'px';
  label.textContent=isPlate?'📋 หันแนวนอน · จัดป้ายให้อยู่ในกรอบ':'🚛 หันแนวนอน · จัดรถให้อยู่ในกรอบ';
...
function closeCam(){
  ...
  document.getElementById('camGuide').classList.remove('show');
}
function snapPhoto(){
